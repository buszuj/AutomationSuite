...
TPTFR_PA_SERVICES = [
    ...
    ['Language Services', 'Translation', 'TM - Exact Matches', 'Word'],
    ['Language Services', 'Translation', 'Localization', 'Hour'],
    ['Desktop Publishing', 'Translation', 'Le Formatting :(', 'Hour'],
    ['Language Services', 'Translation', 'Redaction', 'Hour'],
    ['Language Services', 'Translation', 'Transcription', 'Word'],
    ['Language Services', 'Translation', 'Proofreading', 'Hour'],
    ['Language Services', 'Translation', 'Reconciliation', 'Hour'],
...
def interactive_add_service():
    ...
    print(f"  Service Group 1: {service_group_1}")
    print(f"  Service Group 2: {service_group_2}")
...
def display_services(entity=None):
    ...
    if entity:
        entity_upper = entity.upper()
        if entity_upper in PA_SERVICES:
            print(f"\n{entity_upper} Services:")
            print("-"*80)
            for service in PA_SERVICES[entity_upper]:
                print(f"  {service}")
        else:
            print(f"Entity '{entity}' not found.")
    else:
        for entity_name, services in PA_SERVICES.items():
            print(f"\n{entity_name} Services ({len(services)-1} services):")
            print("-"*80)
