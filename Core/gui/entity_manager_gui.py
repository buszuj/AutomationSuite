...
class EntityManagerGUI:
    ...
    def __init__(self, parent=None, frame=None, on_entity_change=None):
        """
        ...
        Args:
            ...
            on_entity_change: Optional callback called whenever an entity is created or deleted.
        """
    ...
    def setup_creation_panel(self, parent):
        ...
        self.copy_from_combo.set("TPUS" if "TPUS" in existing_entities else "None")
    ...
    def delete_entity(self, entity_name):
        ...
        # Typed confirmation (actual text input)
        typed_name = simpledialog.askstring(
            "Final Confirmation",
            f"LAST CHANCE!\n\n"
            f"Delete '{entity_name}' permanently?\n\n"
            f"Type the entity name exactly to confirm:"
        )

        if typed_name is None:
            return

        if typed_name.strip() != entity_name:
            messagebox.showwarning(
                "Confirmation Failed",
                f"Deletion cancelled. You must type '{entity_name}' exactly."
            )
            return
        ...
        try:
            ...
            # Find and remove entity list definition (exact variable block)
            ...
            if start_line != -1 and end_line != -1:
                # Optionally remove the immediate preceding comment for this exact entity.
                comment_idx = start_line - 1
                if comment_idx >= 0 and lines[comment_idx].strip() == f"# List of {entity_name} PA Services":
                    del lines[comment_idx]
                    start_line -= 1
                    end_line -= 1

                # Remove variable block only.
                del lines[start_line:end_line + 1]
                # Remove a single extra blank line directly after the removed block if present.
                if start_line < len(lines) and lines[start_line].strip() == "":
                    del lines[start_line]
            
            # Remove only this entity from PA_SERVICES dictionary (exact key match)
            new_lines = []
            for line in lines:
                stripped = line.strip()
                if stripped.startswith(f'"{entity_name}":') and "_PA_SERVICES" in stripped:
                    continue
