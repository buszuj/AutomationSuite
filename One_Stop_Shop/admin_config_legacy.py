...
# Define your available labels here (update as needed)
...
MAPPING_FILE = "service_label_mapping.json"
...
def save_service_label_mapping(mapping, account_key):
    all_mappings = load_service_label_mapping()
    all_mappings[account_key] = mapping
    with open(MAPPING_FILE, "w", encoding="utf-8") as f:
        json.dump(all_mappings, f, indent=2)

def open_admin_config(root, FONT, services=None, services_uofm=None, on_save=None, ratesheet_name=None, account_key=None):
    ...
    mapping = load_service_label_mapping()
    all_mappings = load_service_label_mapping()
    mapping = all_mappings.get(account_key, {})

    # Only show word-based and hour-based services
    ...
    def hour_cell(row, col, text="", colspan=1, rowspan=1, bold=False, bg="#f0f0f0"):
        lbl = tk.Label(
            Per_hour_services_table_frame, text=text, borderwidth=hour_borderwidth, relief="solid",
    ...
    # --- GLOBAL MIN HOURLY RATE & INCREMENT RATE ---
    ...
    def save_and_close():
        ...
        if not pm_val:
            pm_val = "10"  # or your preferred default
        new_mapping["default_pm_percent"] = pm_val

        save_service_label_mapping(new_mapping, account_key)
        if on_save:
            on_save()
